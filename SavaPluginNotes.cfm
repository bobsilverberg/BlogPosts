Creating a Sava Plugin from an existing FB/Transfer App

1. Needed to make CS config dynamic, and added the following code to determine the component root:
<cfset componentRoot = request.pluginConfig.getSettings().pluginPath />
<cfif Left(componentRoot,1) EQ "/">
	<cfset componentRoot = Right(componentRoot,Len(componentRoot)-1) />
</cfif>
<cfset componentRoot = Replace(componentRoot,"/",".","all") />

<cfsavecontent variable="servicesXML"><cfoutput>

2. Added this to the front of each bean class attribute:

#componentRoot#

e.g., <bean id="AppConfig" class="model.util.SimpleConfig"> becomes

<bean id="AppConfig" class="#componentRoot#model.util.SimpleConfig">

3. For Transfer beans, had to add a path to the plugin directory as well:
	...
	<bean id="transferFactory" class="transfer.TransferFactory">
		...
		<constructor-arg name="configPath"><value>#request.pluginConfig.getSettings().pluginPath#model/config/transfer.xml.cfm</value></constructor-arg>
		<constructor-arg name="definitionPath"><value>#request.pluginConfig.getSettings().pluginPath#TransferTemp</value></constructor-arg>
	</bean>
...
5. Problem with version of CS that ships with Sava - doesn't support MapFactoryBean.  Replaced with version 1.2.

6. Problem with path to decorators, as above.
- no solution yet

7. When linking from a form - the action can simply be set to "index.cfm" and the FA is OK because it's using a unique URL attribute

8. Needed to add safeCFCInclude.cfm to the requirements/custom_tags/ folder
